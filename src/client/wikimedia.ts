...
export class WikimediaClient implements Wikimedia{
  ...
  public async fetchXmlDesc(location: string): Promise<XmlDesc> {
  ...
  public async fetchMediaType(location: string): Promise<Result<MediaType>> {
  ...
  public async fetchImage(location: string): Promise<Blob> {
  ...
  fileName(location: string): String {
    return location
      .split("/")
      .pop()
      .replace(/^File:/, "")
